...
function feriadoLabel(feriado) {
  ...
  if (diffDays === 0) return ` ⭐ Feriado ${fechaCorta} ${nombre}`;
  const abs = Math.abs(diffDays);
  const dir = diffDays > 0 ? "antes del" : "después del";
  return ` ⭐ ${abs} día${abs > 1 ? "s" : ""} ${dir} feriado ${fechaCorta} ${nombre}`;
}
...
async function findDealsTP() {
  ...
  for (const dest of DESTINOS) {
    ...
    // Acumula todos los candidatos (top 3 por origen/mes)
    const allCandidates = [];
