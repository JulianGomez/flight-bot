...
// ─── Meses de búsqueda por tipo de destino ───────────────────────────────────
function getMonthsForDest(destCode) {
  ...
  const mesesNacionales   = parseInt(process.env.MESES_NACIONALES   || "12");
  const mesesEuropa       = parseInt(process.env.MESES_EUROPA        || "12");
  const mesesNorteamerica = parseInt(process.env.MESES_NORTEAMERICA  || "12");
  const mesesLatam        = parseInt(process.env.MESES_LATAM         || "12");

  if (nacionales.includes(destCode))   return mesesNacionales;
  if (europa.includes(destCode))       return mesesEuropa;
  if (norteamerica.includes(destCode)) return mesesNorteamerica;
  return mesesLatam;
}
...
async function findDealsTP() {
  ...
  for (const dest of DESTINOS) {
    const origins = getOriginsForDest(dest.code);
    const months  = getFutureMonths(getMonthsForDest(dest.code)); // ← meses según tipo
    let bestGlobal = null;
