...
async function searchFlight(token, origin, destination, date) {
  ...
  const timeout = setTimeout(() => controller.abort(), 8000); // 8 segundos máximo
  ...
  if (!res) return null;        // ← timeout o error de red
  if (!res.ok) return null;     // ← respuesta con error HTTP
